...
import { TimelineEvent } from '@emergency-orchestrator/shared';
...
const incidentId = (args.get('incident_id') as string) || (args.get('incidentId') as string);
if (!incidentId) {
...
const interval = parseInt((args.get('interval') as string) ?? '2000', 10);
...
const isTriggerEvent = (payload: Record<string, unknown>) => {
...
async function applyActions(incidentId: string) {
        ...
        try {
            ...
            if (action.type === 'set_state') {
                ...
                const eventType = allowedTimelineTypes.has(action.event_type)
                    ? (action.event_type as TimelineEvent['type'])
                    : ('system_event' as TimelineEvent['type']);
